		...
		if (!isset($this->session->data['api_id'])) {
			...
			if (isset($this->request->post['product'])) {
                ...
                foreach ($this->request->post['product'] as $product) {
                    ...
                    $this->cart->add($product['product_id'], $product['quantity'], $option, isset($product['emi']), $order_product_id);
                }
			}
			...
			if (isset($this->request->post['product_id'])) {
				...
				if ($product_info) {
                    ...
                    if(isset($this->request->post['emi'])) {
                        $isEMIEnabled = $product_info['emi'];
                    }
                    $isEMICart = $this->cart->isEMI();

                    if($isEMIEnabled && !$isEMICart && $this->cart->hasProducts()) {
                        $json['error']['warning'] =   $this->language->get('error_emi_1');
                    } else if(!$isEMIEnabled && $isEMICart && $this->cart->hasProducts()) {
                        $json['error']['warning'] =   $this->language->get('error_emi_2');
                    }
                    ...
                    if (!isset($json['error'])) {
                        $this->cart->add($this->request->post['product_id'], $quantity, $option, $isEMIEnabled);
