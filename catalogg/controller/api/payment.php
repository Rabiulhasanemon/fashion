...
class ControllerApiPayment extends Controller {
	public function address() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ((utf8_strlen(trim($this->request->post['address_1'])) < 3) || (utf8_strlen(trim($this->request->post['address_1'])) > 250)) {
				$json['error']['address_1'] = $this->language->get('error_address_1');
			}

			if ((utf8_strlen($this->request->post['city']) > 32)) {
			...
			if (!$json) {
				...
				$region_info = $this->model_localisation_region->getRegion($this->request->post['region_id']);
				...
				if(isset($this->request->post['same_shipping'])) {
			...
			if (!$json) {
				...
				foreach ($results as $result) {
					if ($this->config->get($result['code'] . '_status')) {
						...
						$method = $this->{'model_payment_' . $result['code']}->getMethod($this->session->data['payment_address'], $total, false);
