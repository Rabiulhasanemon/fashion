...
class ControllerAccountOrder extends Controller {
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_due'] = $this->language->get('text_due');
			...
			$paid = $this->model_checkout_payment->getAmountPaid($order_info['order_id']);
            $due = $order_info['total'] - $paid;
			$data['due'] = $this->currency->format($due, $order_info['currency_code'], $order_info['currency_value']);
			$data['paid'] = $this->currency->format($paid, $order_info['currency_code'], $order_info['currency_value']);
			$data['comment'] = nl2br($order_info['comment']);

			if($due && in_array($order_info['order_status_id'], $this->config->get('config_processing_status'))) {
			    $data['pay'] = $this->url->link('checkout/payment', 'order_id=' . $order_id, 'SSL');
            } else {
                $data['pay'] = null;
            }
