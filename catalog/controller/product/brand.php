<?php
// Brand controller - alias for manufacturer with brand URLs
class ControllerProductBrand extends Controller {
		...
		if ($results) {
			foreach ($results as $result) {
				...
				$data['categories'][$key]['manufacturer'][] = array(
					'name' => $result['name'],
					'image' => $image,
					'href' => $this->url->link('product/brand/info', 'manufacturer_id=' . $manufacturer_id),
	...
	public function info() {
		...
		// Replace all manufacturer URLs with brand URLs
		$output = str_replace('product/manufacturer', 'product/brand', $output);
		$output = str_replace('route=product/manufacturer', 'route=product/brand', $output);
