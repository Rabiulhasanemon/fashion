...
class ControllerBlogCategory extends Controller {
	...
	public function index() {
        ...
        if ($category_info['image']) {
            $data['image'] = $this->model_tool_image->resize($category_info['image'], $this->config->get('config_article_image_category_width'), $this->config->get('config_article_image_category_height'));
        } else {
            $data['image'] = '';
        }
        ...
        if($category_info) {
            ...
            $data['note'] = $category_info['note'];
            $data['description'] = html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8');
            $data['summary'] = html_entity_decode($category_info['summary'], ENT_QUOTES, 'UTF-8');
            ...
            foreach ($results as $article_info) {
                if ($article_info['thumb']) {
                    $image = $this->model_tool_image->resize($article_info['thumb'], $this->config->get("config_article_image_category_width"), $this->config->get("config_article_image_category_height"));
                } else if ($article_info['image']) {
                    $image = $this->model_tool_image->resize($article_info['image'], $this->config->get("config_article_image_category_width"), $this->config->get("config_article_image_category_height"));
                } else {
...
//            }
            if (isset($category_info['view']) && $category_info['view'] && file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/category_' . $category_info['view'] . '.tpl')) {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/category_' . $category_info['view'] . '.tpl', $data));
            } else {
                $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/category.tpl', $data));
            }
