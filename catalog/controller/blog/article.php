...
class ControllerBlogArticle extends Controller {
	...
	public function index() {
		...
		if ($article_info ) {
            ...
            $data['headline_for_details'] = $article_info['headline_for_details'];
            ...
            foreach ($tags as $tag) {
                $data['tags'][] = array(
                    'name' => $tag,
                    'href' => $this->url->link('blog/search', 'tag=' . $tag)
                );
            }
            ...
            if ($article_info['image']) {
                $data['image'] = $this->model_tool_image->resize($article_info['image'], $this->config->get("config_image_article_list_width"), $this->config->get("config_image_article_list_height"));
            } else {
                $data["image"] = null;
            }
            ...
            $this->document->addMeta('og:site_name', "StarTech Service");
    ...
    public function event_stream() {
        ...
        foreach ($results as $result) {
            ...
            foreach ($articles as $article) {
                ...
                if ($article['thumb']) {
                    $image = $this->model_tool_image->resize($article['thumb'], $this->config->get("config_article_image_category_width"), $this->config->get("config_article_image_category_height"));
                } else if ($article['image']) {
                    $image = $this->model_tool_image->resize($article['image'], $this->config->get("config_article_image_category_width"), $this->config->get("config_article_image_category_height"));
                } else {
                    $image = $this->model_tool_image->resize('placeholder.png', $this->config->get("config_article_image_category_width"), $this->config->get("config_article_image_category_height"));
                }
        ...
        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/blog/event_stream.tpl')) {
            $this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/blog/event_stream.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('default/template/blog/event_stream.tpl', $data));
        }
    }
