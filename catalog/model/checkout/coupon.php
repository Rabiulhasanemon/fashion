	...
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			foreach ($coupon_product_query->rows as $product) {
				$coupon_product_data[$product['product_id']] = array(
            ...
            $coupon_category_data = $coupon_category_query->rows;

			$product_data = array();

			if ($coupon_product_data || $coupon_category_data) {
				foreach ($this->cart->getProducts() as $product) {
					if (array_key_exists($product['product_id'], $coupon_product_data)) {
						$product_data[$product['product_id']] = $coupon_product_data[$product['product_id']];
						continue;
					}

					foreach ($coupon_category_data as $category) {
						$coupon_category_query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "product_to_category` WHERE `product_id` = '" . (int)$product['product_id'] . "' AND category_id = '" . (int)$category["category_id"] . "'");
