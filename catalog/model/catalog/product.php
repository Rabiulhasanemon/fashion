...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		$query = $this->db->query("SELECT DISTINCT *, pd.name AS name, p.image, m.name AS manufacturer, m.thumb as manufacturer_thumb, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special, (SELECT points FROM " . DB_PREFIX . "product_reward pr WHERE pr.product_id = p.product_id AND customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "') AS reward, (SELECT ss.name FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS stock_status, (SELECT ss.button FROM " . DB_PREFIX . "stock_status ss WHERE ss.stock_status_id = p.stock_status_id AND ss.language_id = '" . (int)$this->config->get('config_language_id') . "') AS restock_request_btn, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT COUNT(*) AS total FROM " . DB_PREFIX . "review r2 WHERE r2.product_id = p.product_id AND r2.status = '1' GROUP BY r2.product_id) AS reviews, p.sort_order FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) LEFT JOIN " . DB_PREFIX . "manufacturer m ON (p.manufacturer_id = m.manufacturer_id) WHERE p.product_id = '" . (int)$product_id . "' AND pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'");
        if ($query->num_rows) {
            $price = $query->row['discount'] ? $query->row['discount'] : $query->row['price'];
			return array(
				'product_id'       => $query->row['product_id'],
				'parent_id'       => $query->row['parent_id'],
				'is_manufacturer_is_parent' => $query->row['is_manufacturer_is_parent'],
				'name'             => $query->row['name'],
				...
				'sku'              => $query->row['sku'],
				'mpn'              => $query->row['mpn'],
				'short_note'       => $query->row['short_note'],
				'quantity'         => $query->row['quantity'],
				'stock_status_id'  => $query->row['stock_status_id'],
				'stock_status'     => $query->row['stock_status'],
				'restock_request_btn'     => $query->row['restock_request_btn'],
				'image'            => $query->row['image'],
				'featured_image'   => $query->row['featured_image'],
				'manufacturer_id'  => $query->row['manufacturer_id'],
				'manufacturer'     => $query->row['manufacturer'],
				'manufacturer_thumb' => $query->row['manufacturer_thumb'],
				'price'            => $price,
				'emi'              => $query->row['emi'],
				'special'          => $query->row['special'],
				'regular_price'    => $query->row['regular_price'],
				...
				'rating'           => $query->row['rating'] !== null ? round((float)$query->row['rating']) : 0,
	...
	public function getProducts($data = array()) {
        ...
        if (!empty($data['filter_filter'])) {
            ...
            foreach ($filter_groups as $filter) {
                ...
                $sql .= " AND p.product_id IN (SELECT pf.product_id FROM ". DB_PREFIX. "product_filter pf where pf.filter_id IN (" . implode(',', $implode) . "))";
            }
        }
	...
	public function getBestSellerProducts($limit, $parent_id = 0) {
	    $cache_key = 'product.bestseller.' . (int)$this->config->get('config_language_id') . '.' . (int)$this->config->get('config_store_id') . '.' . $this->config->get('config_customer_group_id');
	    if($parent_id) {
	        $cache_key .= $parent_id;
        }
	    $cache_key .= '.' . (int)$limit;
		...
		if (!$product_data) {
			...
			if($parent_id) {
			    $sql .= "' AND p.parent_id = '" . (int) $parent_id;
            }
    ...
    public function getProductVariation($product_id, $key) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_variation WHERE product_id = '" . (int)$product_id . "' AND `key` = '" . $this->db->escape($key) . " '");
    ...
    public function getProductOptions($product_id) {
        ...
        foreach ($product_option_query->rows as $product_option) {
            ...
            $product_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.product_id = '" . (int)$product_id . "' AND pov.product_option_id = '" . (int)$product_option['product_option_id'] . "' AND pov.show = 1 AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
	...
	public function getProductStockStatuses() {
    ...
    public function getPriceRange($data = array()) {
        ...
        } elseif (!empty($data['filter_categories'])) {
            if (!empty($data['filter_sub_category'])) {
