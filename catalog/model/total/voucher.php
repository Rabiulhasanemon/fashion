...
class ModelTotalVoucher extends Model {
	...
	public function confirm($order_info, $order_total) {
		...
		$voucher_info = $this->model_checkout_voucher->getVoucher($code, false);
		if ($voucher_info) {
	...
	public function unconfirm($order_id, $order_total) {
        ...
        if ($voucher_info) {
            $this->db->query("INSERT INTO `" . DB_PREFIX . "voucher_history` SET voucher_id = '" . (int)$voucher_info['voucher_id'] . "', order_id = '" . (int) $order_id . "', amount = '" . (float)-$order_total['value'] . "', date_added = NOW()");
        }
	}
}