...
class ModelSaleCustomer extends Model {
	public function addCustomer($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id']
            ...
            . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'] ? $data['password'] : "b@byc@r3"))))
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET customer_id = '" . (int)$customer_id
                    . "', firstname = '" . $this->db->escape(trim($address['firstname']) ? $address['firstname'] : $data['firstname'])
                    . "', lastname = '" . $this->db->escape(trim($address['lastname']) ? $address['lastname'] : $data['lastname'])
	...
	public function editCustomer($customer_id, $data) {
		...
		if (isset($data['address'])) {
			foreach ($data['address'] as $address) {
				...
				$this->db->query("INSERT INTO " . DB_PREFIX . "address SET address_id = '" . (int)$address['address_id']
	...
	public function getCustomerByTelephone($telephone) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "customer WHERE telephone = '" . $this->db->escape($telephone) . "'");
	...
	public function getCustomers($data = array()) {
		...
		if (!empty($data['filter_search'])) {
			$implode[] = "(CONCAT(c.firstname, ' ', c.lastname) LIKE '%" . $this->db->escape($data['filter_search']) . "%' OR c.email LIKE '%" . $this->db->escape($data['filter_search']). "%' OR c.telephone LIKE '%" . $this->db->escape($data['filter_search']) . "%')";
		}
	...
	public function getAddress($address_id) {
		...
		if ($address_query->num_rows) {
			...
			$region_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "region` WHERE region_id = '" . (int)$address_query->row['region_id'] . "'");

			if ($region_query->num_rows) {
				$region = $region_query->row['name'];
				$region_code = $region_query->row['code'];
			} else {
				$region = '';
				$region_code = '';
			}
			...
			return array(
				...
				'zone'           => $zone,
				'zone_code'      => $zone_code,
				'region_id'        => $address_query->row['region_id'],
				'region'           => $region,
	...
	public function getTotalAddressesByRegionId($region_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "address WHERE region_id = '" . (int)$region_id . "'");
