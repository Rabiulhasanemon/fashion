	...
	public function addLayout($data) {
		...
		if (isset($data['layout_route'])) {
			foreach ($data['layout_route'] as $layout_route) {
				// Skip empty routes (no route specified)
				if (!empty($layout_route['route'])) {
		...
		if (isset($data['layout_module'])) {
			foreach ($data['layout_module'] as $layout_module) {
				...
				if (!empty($layout_module['code'])) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "layout_module SET layout_id = '" . (int)$layout_id . "', code = '" . $this->db->escape($layout_module['code']) . "', position = '" . $this->db->escape($layout_module['position']) . "', device = '" . $this->db->escape($layout_module['device']) . "', sort_order = '" . (int)$layout_module['sort_order'] . "'");
				}
			}
		}
