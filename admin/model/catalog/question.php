...
class ModelCatalogQuestion extends Model {
	public function addQuestion($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "question SET author = '" . $this->db->escape($data['author']) . "', email = '" . $this->db->escape($data['email'])  . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', answer = '" . $this->db->escape(strip_tags($data['answer'])) . "', status = '" . (int)$data['status']. "', `show` = '" . (int)$data['show'] . "', date_added = NOW()");
	...
	public function editQuestion($question_id, $data) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "question SET author = '" . $this->db->escape($data['author']) . "', email = '" . $this->db->escape($data['email']) . "', product_id = '" . (int)$data['product_id'] . "', text = '" . $this->db->escape(strip_tags($data['text'])) . "', answer = '" . $this->db->escape(strip_tags($data['answer'])) . "', status = '" . (int)$data['status'] . "', `show` = '" . (int)$data['show'] . "', date_modified = NOW() WHERE question_id = '" . (int)$question_id . "'");
        ...
        $message = sprintf($this->language->get('text_product'), html_entity_decode($data['product'], ENT_QUOTES, 'UTF-8')) . "\n";
        $message .= sprintf($this->language->get('text_question'), html_entity_decode($data['text'], ENT_QUOTES, 'UTF-8')) . "\n";
        $message .= sprintf($this->language->get('text_answer'), html_entity_decode($data['answer'], ENT_QUOTES, 'UTF-8')) . "\n";
	...
	public function getQuestions($data = array()) {
		$sql = "SELECT r.question_id, pd.name, r.author, r.text, r.answer, r.status, r.date_added FROM " . DB_PREFIX . "question r LEFT JOIN " . DB_PREFIX . "product_description pd ON (r.product_id = pd.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
