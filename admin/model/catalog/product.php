...
class ModelCatalogProduct extends Model {
	...
	public function addProduct($data) {
		...
		// Log what data keys are present
		$data_keys = array_keys($data);
		file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Data keys received: ' . implode(', ', $data_keys) . PHP_EOL, FILE_APPEND);
		
		// Log specific tab data with detailed information
		$tab_data = array(
			'product_description' => isset($data['product_description']) ? 'YES (' . count($data['product_description']) . ' languages)' : 'NO',
			'product_category' => isset($data['product_category']) ? 'YES (' . count($data['product_category']) . ' categories)' : 'NO',
			'product_filter' => isset($data['product_filter']) ? 'YES (' . count($data['product_filter']) . ' filters)' : 'NO',
			'product_attribute' => isset($data['product_attribute']) ? 'YES (' . count($data['product_attribute']) . ' attributes)' : 'NO',
			'product_option' => isset($data['product_option']) ? 'YES (' . count($data['product_option']) . ' options)' : 'NO',
			'product_discount' => isset($data['product_discount']) ? 'YES (' . count($data['product_discount']) . ' discounts)' : 'NO',
			'product_special' => isset($data['product_special']) ? 'YES (' . count($data['product_special']) . ' specials)' : 'NO',
			'product_image' => isset($data['product_image']) ? 'YES (' . count($data['product_image']) . ' images)' : 'NO',
			'product_reward' => isset($data['product_reward']) ? 'YES (' . count($data['product_reward']) . ' rewards)' : 'NO',
			'product_download' => isset($data['product_download']) ? 'YES (' . count($data['product_download']) . ' downloads)' : 'NO',
			'product_layout' => isset($data['product_layout']) ? 'YES (' . count($data['product_layout']) . ' layouts)' : 'NO'
		);
		
		// CRITICAL: Detailed logging for filters and attributes
		if (isset($data['product_filter'])) {
			if (is_array($data['product_filter']) && !empty($data['product_filter'])) {
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_filter: Array with ' . count($data['product_filter']) . ' items: ' . implode(', ', $data['product_filter']) . PHP_EOL, FILE_APPEND);
			} else {
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_filter: ' . (is_array($data['product_filter']) ? 'EMPTY array' : 'NOT an array: ' . gettype($data['product_filter'])) . PHP_EOL, FILE_APPEND);
			}
		} else {
			file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_filter: NOT FOUND in data' . PHP_EOL, FILE_APPEND);
		}
		
		if (isset($data['product_attribute'])) {
			if (is_array($data['product_attribute']) && !empty($data['product_attribute'])) {
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_attribute: Array with ' . count($data['product_attribute']) . ' items' . PHP_EOL, FILE_APPEND);
				foreach ($data['product_attribute'] as $key => $attr) {
					$attr_id = isset($attr['attribute_id']) ? $attr['attribute_id'] : (is_numeric($key) ? $key : 'unknown');
					$has_text = false;
					if (isset($attr['product_attribute_description']) && is_array($attr['product_attribute_description'])) {
						foreach ($attr['product_attribute_description'] as $lang_id => $desc) {
							if (isset($desc['text']) && !empty(trim($desc['text']))) {
								$has_text = true;
								file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT]   Attribute ' . $attr_id . ', Language ' . $lang_id . ': text length = ' . strlen($desc['text']) . PHP_EOL, FILE_APPEND);
							}
						}
					}
					if (!$has_text) {
						file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT]   Attribute ' . $attr_id . ': NO TEXT FOUND' . PHP_EOL, FILE_APPEND);
					}
				}
			} else {
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_attribute: ' . (is_array($data['product_attribute']) ? 'EMPTY array' : 'NOT an array: ' . gettype($data['product_attribute'])) . PHP_EOL, FILE_APPEND);
			}
		} else {
			file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [INIT] product_attribute: NOT FOUND in data' . PHP_EOL, FILE_APPEND);
		}
		
		file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Tab data status: ' . print_r($tab_data, true) . PHP_EOL, FILE_APPEND);
		...
		if ($table_check && $table_check->num_rows > 0) {
			if (isset($data['product_frequently_bought_together']) && is_array($data['product_frequently_bought_together'])) {
				...
				try {
					...
					file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [FBT] Error deleting: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
				}
				$sort_order = 0;
				foreach ($data['product_frequently_bought_together'] as $fbt_product_id) {
					...
					if ($fbt_product_id > 0 && $fbt_product_id != $product_id) {
						try {
							...
							file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [FBT] Error inserting product ' . $fbt_product_id . ': ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
						}
						$sort_order++;
					}
				}
			...
			file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [FBT] Table does not exist, skipping FBT operations' . PHP_EOL, FILE_APPEND);
		}
		...
		if ($product_id > 0) {
			...
			try {
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Warning cleaning product_discount: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
			}
		...
		if ($product_id > 0) {
			...
			try {
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Warning cleaning product_special: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
			}
		...
		if ($product_id > 0) {
			...
			try {
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Warning cleaning product_reward: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
			}
		...
		if ($product_id > 0) {
			...
			try {
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Warning cleaning product_to_download: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
			}
		...
		if ($product_id > 0) {
			...
			try {
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - Warning cleaning product_to_layout: ' . $e->getMessage() . PHP_EOL, FILE_APPEND);
			}
	...
	public function editProduct($product_id, $data) {
		...
		// Update Frequently Bought Together products
		// Check if table exists first to avoid fatal errors
		$table_check = $this->db->query("SHOW TABLES LIKE '" . DB_PREFIX . "product_frequently_bought_together'");
		...
		if ($table_check && $table_check->num_rows > 0) {
			// Always delete existing FBT records first (even if no new ones are being added)
			try {
				$this->db->query("DELETE FROM " . DB_PREFIX . "product_frequently_bought_together WHERE product_id = '" . (int)$product_id . "'");
			...
			// Insert new FBT products if provided
			if (isset($data['product_frequently_bought_together']) && is_array($data['product_frequently_bought_together']) && count($data['product_frequently_bought_together']) > 0) {
					...
					if ($fbt_product_id > 0 && $fbt_product_id != $product_id) {
						try {
							...
							if ($insert_result !== false) {
								...
								if ($inserted_id > 0) {
									$inserted_count++;
									file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ✓ Successfully inserted FBT product ID: ' . $fbt_product_id . ' (inserted_id: ' . $inserted_id . ', sort_order: ' . $sort_order . ')' . PHP_EOL, FILE_APPEND);
								} else {
									// Check if record was inserted anyway (might be duplicate)
									$check_query = $this->db->query("SELECT id FROM " . DB_PREFIX . "product_frequently_bought_together WHERE product_id = '" . (int)$product_id . "' AND fbt_product_id = '" . $fbt_product_id . "' LIMIT 1");
									if ($check_query && $check_query->num_rows > 0) {
										$inserted_count++;
										file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ✓ FBT product ID: ' . $fbt_product_id . ' already exists (duplicate), counted as inserted' . PHP_EOL, FILE_APPEND);
									} else {
										file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ✗ Failed to insert FBT product ID: ' . $fbt_product_id . ' (query returned false, no record found)' . PHP_EOL, FILE_APPEND);
									}
								}
							} else {
								// Query failed, check for duplicate key error
								$check_query = $this->db->query("SELECT id FROM " . DB_PREFIX . "product_frequently_bought_together WHERE product_id = '" . (int)$product_id . "' AND fbt_product_id = '" . $fbt_product_id . "' LIMIT 1");
								if ($check_query && $check_query->num_rows > 0) {
									$inserted_count++;
									file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ✓ FBT product ID: ' . $fbt_product_id . ' already exists (duplicate key), counted as inserted' . PHP_EOL, FILE_APPEND);
								} else {
							...
							// Check if it was inserted anyway
							$check_query = $this->db->query("SELECT id FROM " . DB_PREFIX . "product_frequently_bought_together WHERE product_id = '" . (int)$product_id . "' AND fbt_product_id = '" . $fbt_product_id . "' LIMIT 1");
							if ($check_query && $check_query->num_rows > 0) {
								$inserted_count++;
								file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ✓ FBT product ID: ' . $fbt_product_id . ' exists despite exception, counted as inserted' . PHP_EOL, FILE_APPEND);
							}
						}
						$sort_order++;
					} else {
						file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] ⚠ Skipped invalid FBT product ID: ' . $fbt_product_id . ' (must be > 0 and != product_id)' . PHP_EOL, FILE_APPEND);
					}
				}
				...
				file_put_contents($log_file, date('Y-m-d H:i:s') . ' - [EDIT-FBT] No FBT data or empty array - all existing FBT records have been deleted' . PHP_EOL, FILE_APPEND);
			}
		} else {
