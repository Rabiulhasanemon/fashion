	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['questions'][] = array(
				...
				'name'       => $result['name'],
				'question'   => html_entity_decode($result['text'], ENT_QUOTES, 'UTF-8'),
		...
		$data['column_question'] = $this->language->get('column_author');
		...
		$data['sort_product'] = $this->url->link('catalog/question', 'token=' . $this->session->data['token'] . '&sort=pd.name' . $url, 'SSL');
		$data['sort_author'] = $this->url->link('catalog/question', 'token=' . $this->session->data['token'] . '&sort=r.author' . $url, 'SSL');
		$data['sort_status'] = $this->url->link('catalog/question', 'token=' . $this->session->data['token'] . '&sort=r.status' . $url, 'SSL');
		$data['sort_date_added'] = $this->url->link('catalog/question', 'token=' . $this->session->data['token'] . '&sort=r.date_added' . $url, 'SSL');
