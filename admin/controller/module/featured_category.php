	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('featured_category', $this->request->post);
			} else {
		...
		$data['entry_child_limit'] = $this->language->get('entry_child_limit');
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('module/featured_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);
		}
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['action'] = $this->url->link('module/featured_category', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['child'])) {
			...
			$data['child'] = $module_info['child'];
		} else {
			$data['child'] = 0;
		}

		if (isset($this->request->post['child_limit'])) {
			$data['child_limit'] = $this->request->post['child_limit'];
		} elseif (!empty($module_info)) {
			$data['child_limit'] = $module_info['child_limit'];
		} else {
			$data['child_limit'] = '';
		}
