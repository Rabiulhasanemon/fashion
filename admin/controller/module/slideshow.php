...
class ControllerModuleSlideshow extends Controller {
	...
	public function index() {
		...
		// Load banner images for preview if banner_id is set
		$data['banner_preview'] = array();
		if (!empty($data['banner_id'])) {
			$banner_images = $this->model_design_banner->getBannerImages($data['banner_id']);
			$language_id = $this->config->get('config_language_id');
			$preview_width = !empty($data['width']) ? (int)$data['width'] : 1920;
			$preview_height = !empty($data['height']) ? (int)$data['height'] : 600;
			
			foreach ($banner_images as $banner_image) {
				if (is_file(DIR_IMAGE . $banner_image['image'])) {
					$title = '';
					if (isset($banner_image['banner_image_description'][$language_id]['title'])) {
						$title = $banner_image['banner_image_description'][$language_id]['title'];
					}
					
					$data['banner_preview'][] = array(
						'title' => $title,
						'link'  => isset($banner_image['link']) ? $banner_image['link'] : '',
						'image' => $this->model_tool_image->resize($banner_image['image'], $preview_width, $preview_height)
					);
				}
			}
		}
	...
	protected function validate() {
		...
		if ((utf8_strlen($this->request->post['name']) < 3) || (utf8_strlen($this->request->post['name']) > 500)) {
