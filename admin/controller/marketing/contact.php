...
class ControllerMarketingContact extends Controller {
	...
	public function index() {
		...
		$data['text_newsletter_customer'] = $this->language->get('text_newsletter_customer');
			...
			if (!$json) {
				...
				switch ($this->request->post['to']) {
                    ...
                    case 'newsletter':
                        ...
                        $email_total = $this->model_marketing_contact->getTotalSubscribers($subscriber_data);
                        $results = $this->model_marketing_contact->getSubscribers($subscriber_data);

                        foreach ($results as $result) {
                            $emails[] = $result['email'];
                        }
                        break;
					case 'newsletter_customer':
						$customer_data = array(
							'filter_newsletter' => 1,
							'start'             => ($page - 1) * 10,
							'limit'             => 10
