...
class ControllerMarketingCoupon extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_user_user->addActivity($this->user->getId(), 'add_coupon', $activity_data, $coupon_id);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->model_user_user->addActivity($this->user->getId(), 'edit_coupon', $activity_data, $this->request->get['coupon_id']);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $coupon_id) {
                ...
                $this->model_user_user->addActivity($this->user->getId(), 'delete_coupon', $activity_data, $coupon_id);
			}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['max_total'])) {
			...
			$data['max_total'] = $coupon_info['max_total'];
		} else {
			$data['max_total'] = '';
		}
		...
		foreach ($products as $product) {
			$product_info = $this->model_catalog_product->getProduct($product['product_id'] ?: 0);
