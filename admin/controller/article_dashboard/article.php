<?php
class ControllerArticleDashboardArticle extends Controller {
	public function index() {
		$this->load->language('article_dashboard/article');
		...
		$today = $this->model_blog_article->getTotalArticles(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_blog_article->getTotalArticles(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		...
		return $this->load->view('article_dashboard/article.tpl', $data);
	}
}