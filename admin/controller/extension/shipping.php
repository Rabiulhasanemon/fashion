...
class ControllerExtensionShipping extends Controller {
	...
	public function uninstall() {
		...
		if ($this->validate()) {
            ...
            $this->model_extension_shipping->deleteShippingsByCode($this->request->get['extension']);
    ...
    public function delete() {
        ...
        if (isset($this->request->get['shipping_id']) && $this->validateDelete()) {
            $this->model_extension_shipping->deleteShipping($this->request->get['shipping_id']);
	...
	public function getList() {
		...
		if ($files) {
			foreach ($files as $file) {
                ...
                $shippings = $this->model_extension_shipping->getShippingsByCode($extension);

                foreach ($shippings as $shipping) {
                    $shipping_data[] = array(
                        'shipping_id' => $shipping['shipping_id'],
                        'name'      => $this->language->get('heading_title') . ' &gt; ' . $shipping['name'],
                        'edit'      => $this->url->link('shipping/' . $extension, 'token=' . $this->session->data['token'] . '&shipping_id=' . $shipping['shipping_id'], 'SSL'),
                        'delete'    => $this->url->link('extension/shipping/delete', 'token=' . $this->session->data['token'] . '&shipping_id=' . $shipping['shipping_id'], 'SSL')
                    );
                }
