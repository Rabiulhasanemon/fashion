...
class ControllerLocalisationStockStatus extends Controller {
	...
	protected function getForm() {
		...
		$data['entry_request_success'] = $this->language->get('entry_request_success');
        ...
        if (isset($this->error['request_success'])) {
            $data['error_request_success'] = $this->error['request_success'];
        } else {
            $data['error_request_success'] = array();
        }
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['stock_status'] as $language_id => $value) {
            ...
            if (utf8_strlen($value['button']) > 32) {
                $this->error['button'][$language_id] = $this->language->get('error_button');
            }

            if (utf8_strlen($value['request_success']) > 150) {
                $this->error['request_success'][$language_id] = $this->language->get('error_request_success');
            }
		}
