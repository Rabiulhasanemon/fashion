...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
        ...
        if (isset($this->request->get['filter_assignee_id'])) {
            $filter_assignee_id = $this->request->get['filter_assignee_id'];
        } else {
        ...
        if (isset($this->request->get['filter_assignee_id'])) {
            $url .= '&filter_assignee_id=' . $this->request->get['filter_assignee_id'];
        }
		...
		$data['order_history'] = $this->url->link('sale/order_history', 'token=' . $this->session->data['token'], 'SSL');
		...
		foreach ($results as $result) {
            $last_assign_history = $this->model_sale_order->getOrderLastAssignHistory($result['order_id']);
            $sss = null;
			$data['orders'][] = array(
				...
				'last_comment' => $last_assign_history ? $last_assign_history["comment"] : "",
				...
				'feedback'        => $this->url->link('sale/order_feedback/add', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}
		...
		$data['entry_assignee'] = $this->language->get('entry_assignee');
		...
		$data['button_order_history'] = $this->language->get('button_order_history');
		...
		$data['filter_assignee_id'] = $filter_assignee_id;
		$data['filter_customer'] = $filter_customer;
		$data['filter_order_status'] = isset($this->request->get['filter_order_status']) ? $this->request->get['filter_order_status'] : null;
		...
		if($filter_assignee_id) {
		    ...
		    $assignee_info = $this->model_user_user->getUser($filter_assignee_id);
		    $data['filter_assignee'] = $assignee_info['firstname'] . ' ' . $assignee_info['lastname'];
        } else {
            $data['filter_assignee'] = '';
        }
	...
	public function getForm() {
		...
		$data['entry_region_code'] = $this->language->get('entry_region_code');
		$data['entry_zone'] = $this->language->get('entry_zone');
		$data['entry_zone_code'] = $this->language->get('entry_zone_code');
		$data['entry_country'] = $this->language->get('entry_country');
		$data['entry_same_shipping'] = $this->language->get('entry_same_shipping');
		...
		if (!empty($order_info)) {
			...
			$data['payment_region_id'] = $order_info['payment_region_id'];
			...
			$data['shipping_region_id'] = $order_info['shipping_region_id'];
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['text_region_code'] = $this->language->get('text_region_code');
			...
			$data['text_loading'] = $this->language->get('text_loading');
			$data['text_emi'] = $this->language->get('text_emi');
			$data['text_emi_tenure'] = $this->language->get('text_emi_tenure');
			...
			$data['button_change_assignee'] = $this->language->get('button_change_assignee');
			...
			$data['tab_assign_history'] = $this->language->get('tab_assign_history');
			$data['tab_payment_history'] = $this->language->get('tab_payment_history');
			...
			$data['lastname'] = $order_info['lastname'];
			$data['emi'] = $order_info['emi'];
			$data['emi_tenure'] = sprintf($this->language->get("emi_tenure"), $order_info['emi_tenure']);
			...
			$data['payment_city'] = $order_info['payment_city'];
			$data['payment_postcode'] = $order_info['payment_postcode'];
			$data['payment_region'] = $order_info['payment_region'];
			$data['payment_region_code'] = $order_info['payment_region_code'];
			...
			$data['shipping_city'] = $order_info['shipping_city'];
			$data['shipping_postcode'] = $order_info['shipping_postcode'];
			$data['shipping_region'] = $order_info['shipping_region'];
			$data['shipping_region_code'] = $order_info['shipping_region_code'];
            ...
            $data['users'] = $this->model_user_user->getUsers(array('status' => 1));
    ...
    public function addPayment() {
        ...
        if($this->validate()) {
            ...
            $tracking = "Manually Added By " . $user_info["firstname"] . " " . $user_info["lastname"] . " - " . $user_info["user_id"];

            $this->load->language('payment/' . $this->request->post['payment_method']);
            $data = array(
                'order_id' => $this->request->get['order_id'],
                'status' => $this->request->post['payment_status'],
                'gateway_code' => $this->request->post['payment_method'],
                'gateway_title' => $this->language->get('heading_title'),
            ...
            $this->load->model("sale/order");
            $this->model_sale_order->addPayment($data);

            if($this->request->post['order_status_id']) {
                $this->request->get['api'] = "api/order/history";
                $this->request->post["user_id"] = $user_info["user_id"];
                $this->request->post["notify"] = isset($this->request->post["notify"]) ? 1 : 0;
                $this->api();
            }

            $json['success'] = $this->language->get("text_success_add_payment");
        } else {
    ...
    public function payment_history() {
        ...
        $data['column_date_added'] = $this->language->get('column_date_added');
        $data['column_payment_gateway'] = $this->language->get('column_payment_gateway');
        $data['column_status'] = $this->language->get('column_status');
        $data['column_tracking'] = $this->language->get('column_tracking');
        $data['column_transaction_id'] = $this->language->get('column_transaction_id');
        $data['column_payer_info'] = $this->language->get('column_payer_info');
        $data['column_comment'] = $this->language->get('column_comment');
        $data['column_total'] = $this->language->get('column_total');
        ...
        $results = $this->model_sale_order->getOrderPayments($this->request->get['order_id'], ($page - 1) * 10, 10);
        ...
        $payment_total = $this->model_sale_order->getTotalOrderPayments($this->request->get['order_id']);

        $pagination = new Pagination();
        $pagination->total = $payment_total;
        $pagination->page = $page;
        $pagination->limit = 10;
        $pagination->url = $this->url->link('sale/order/payment_history', 'token=' . $this->session->data['token'] . '&order_id=' . $this->request->get['order_id'] . '&page={page}', 'SSL');
        ...
        $this->response->setOutput($this->load->view('sale/order_payment.tpl', $data));
    }
	...
	public function invoice() {
		...
		$data['text_payment_method'] = $this->language->get('text_payment_method');
		$data['text_shipping_method'] = $this->language->get('text_shipping_method');
        $data['text_footer'] = sprintf($this->language->get('text_footer'), $this->config->get("config_email"), $this->config->get("config_telephone"));
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($store_info) {
                    ...
                    $store_logo = HTTPS_CATALOG . 'image/' . $this->config->get('config_logo');
				} else {
				...
				$replace = array(
					...
					'region'      => $order_info['payment_region'],
					'region_code' => $order_info['payment_region_code'],
				...
				$replace = array(
					...
					'region'      => $order_info['shipping_region'],
					'region_code' => $order_info['shipping_region_code'],
                ...
                $total_data[] = array(
                    'title' => $this->language->get("text_amount_paid"),
                    'text' =>  $this->currency->format($order_info['amount_paid'], $order_info['currency_code'], $order_info['currency_value']),
                );

                $total_data[] = array(
                    'title' => $this->language->get("text_due"),
                    'text' =>  $this->currency->format($order_info['amount_due'], $order_info['currency_code'], $order_info['currency_value']),
                );
				...
				$data['orders'][] = array(
					...
					'total'              => $total_data,
					'due'                => $this->currency->format($order_info['amount_due'], $order_info['currency_code'], $order_info['currency_value']),
					'comment'            => nl2br($order_info['comment'])
				);

				if($this->config->get("config_order_print_status_id")) {
				    $this->log->write($this->load->controller("sale/order_history/addHistory", array(
				        'order_id' => $order_id,
				        'notify' => false,
				        'order_status_id' => $this->config->get("config_order_print_status_id")
                    )));
                }
			}
		}
	...
	public function shipping() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info && $order_info['shipping_code']) {
                ...
                $last_history = $this->model_sale_order->getOrderLastHistory($order_info['order_id']);
				...
				$data['orders'][] = array(
					...
					'product'          => $product_data,
					'comment'          => nl2br($order_info['comment']),
                    'last_comment'  => isset($last_history['comment']) ? $last_history['comment'] : ""
				);
			}
		}
	...
	public function report() {
        ...
        if (isset($this->request->get['filter_assignee_id'])) {
            $filter_assignee_id = array_map(function ($v) {
                return (int) $v;
            }, explode(',', $this->request->get['filter_assignee_id']));
        } else {
            $filter_assignee_id = null;
        }
        ...
        foreach ($results as $result) {
            ...
            $histories = $this->model_sale_order->getOrderAssignHistories($result['order_id'], 0, 100);
            $comment = array_column($histories, 'comment');
        ...
        $data['total_amount'] = $this->currency->format($total_amount, $result['currency_code'], $result['currency_value']);
        ...
        echo $this->load->view('sale/order_report.tpl', $data);
        exit();
    }
}