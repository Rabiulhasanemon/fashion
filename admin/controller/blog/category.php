...
class ControllerBlogCategory extends Controller {
	...
	protected function getList() {
		...
		$data['repair'] = $this->url->link('clog/category/repair', 'token=' . $this->session->data['token'] . $url, true);
		...
		foreach ($results as $result) {
			$data['categories'][] = array(
				...
				'edit'        => $this->url->link('blog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true),
				'delete'      => $this->url->link('blog/category/delete', 'token=' . $this->session->data['token'] . '&category_id=' . $result['category_id'] . $url, true)
			);
		}
		...
		$data['sort_name'] = $this->url->link('blog/category', 'token=' . $this->session->data['token'] . '&sort=name' . $url, true);
		$data['sort_sort_order'] = $this->url->link('blog/category', 'token=' . $this->session->data['token'] . '&sort=sort_order' . $url, true);
		...
		$pagination->url = $this->url->link('blog/category', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
	...
	protected function getForm() {
        ...
        $data['entry_local'] = $this->language->get('entry_local');
		...
		if (!isset($this->request->get['category_id'])) {
			...
			$data['action'] = $this->url->link('blog/category/edit', 'token=' . $this->session->data['token'] . '&category_id=' . $this->request->get['category_id'] . $url, true);
		}
        ...
        if (isset($this->request->post['local'])) {
            $data['local'] = $this->request->post['local'];
        } elseif (!empty($category_info)) {
            $data['local'] = $category_info['local'];
        } else {
            $data['local'] = 0;
        }
	...
	protected function validateForm() {
		...
		foreach ($this->request->post['category_description'] as $language_id => $value) {
            ...
            if ((utf8_strlen($value['blurb']) < 2) || (utf8_strlen($value['blurb']) > 500)) {
