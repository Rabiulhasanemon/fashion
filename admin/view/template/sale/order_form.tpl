        ...
        <form class="form-horizontal">
          ...
          <div class="tab-content">
                ...
                <table class="table table-bordered">
                  ...
                  <tr>
                    <td class="text-left"><?php echo isset($order_product['name']) ? $order_product['name'] : ''; ?><br />
                      ...
                      <input type="hidden" name="product[<?php echo $product_row; ?>][option][<?php echo $option['product_option_id']; ?>]" value="<?php echo $option['option_value_id']; ?>" />
                      <?php } ?></td>
              ...
              <div class="tab-content">
                <div class="tab-pane active" id="tab-product">
                  <fieldset>
                    ...
                    <?php if($emi) { ?>
                    <input type="hidden" name="emi" value="1" />
                    <?php } ?>
                    <div class="form-group">
            ...
            <div class="tab-pane" id="tab-payment">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-shipping-region"><?php echo $entry_region; ?></label>
                <div class="col-sm-10">
                  <select name="region_id" id="input-shipping-region" class="form-control">
                  </select>
                </div>
              </div>

			  <div class="form-group">
                <label class="col-sm-2 control-label" for="input-same-shipping-address"><?php echo $entry_same_shipping; ?></label>
                <div class="col-sm-10">
                  <div class="checkbox">
                    <label><input type="checkbox" checked class="form-control" name="same_shipping" id="input-same-shipping-address"></label>
                  </div>
                </div>
              </div>
            ...
            <div class="tab-pane" id="tab-total">
              ...
              <div class="row">
                <div class="col-sm-6 text-left">
                  <button type="button" onclick="$('select[name=\'shipping_method\']').prop('disabled') || $('input[name=\'same_shipping\']').prop('checked') ? $('a[href=\'#tab-payment\']').tab('show') : $('a[href=\'#tab-shipping\']').tab('show');" class="btn btn-default"><i class="fa fa-arrow-left"></i> <?php echo $button_back; ?></button>
                </div>
                <div class="col-sm-6 text-right">
...
var emi = Number.parseInt('<?php echo $emi; ?>');
// Disable the tabs
		...
		success: function(json) {
			...
			if (json['products']) {
				for (i = 0; i < json['products'].length; i++) {
					...
					if(emi) {
                      html += '  <input type="hidden" name="product[' + i + '][emi]" value="1" />';
                    }
					if (product['option']) {
...
$('input[name=\'customer\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=sale/customer/autocomplete&token=<?php echo $token; ?>&filter_search=' +  encodeURIComponent(request),
			dataType: 'json',			
			success: function(json) {
				json.unshift({
					customer_id: '0',
...
$('#button-customer').on('click', function() {
    ...
    $('#button-product-add').on('click', function() {
      $.ajax({
        url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/cart/add&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
        type: 'post',
        data: $('#tab-product input[name=\'emi\'], #tab-product input[name=\'product_id\'], #tab-product input[name=\'quantity\'], #tab-product input[name^=\'option\'][type=\'text\'], #tab-product input[name^=\'option\'][type=\'hidden\'], #tab-product input[name^=\'option\'][type=\'radio\']:checked, #tab-product input[name^=\'option\'][type=\'checkbox\']:checked, #tab-product select[name^=\'option\'], #tab-product textarea[name^=\'option\']'),
        dataType: 'json',
        beforeSend: function() {
          $('#button-product-add').button('loading');
        },
...
$('select[name=\'payment_address\']').on('change', function() {
	$.ajax({
		...
		success: function(json) {
			...
			payment_region_id = json['region_id'];
...
var payment_region_id = '<?php echo $payment_region_id; ?>';
    ...
    success: function(json) {
      ...
      if (json['region'] && json['region'] != '') {
        ...
        for (i = 0; i < json['region'].length; i++) {
          ...
          if (json['region'][i]['region_id'] == payment_region_id) {
            html += ' selected="selected"';
          }
      ...
      $('#tab-payment select[name=\'region_id\']').html(html);
    },
    error: function(xhr, ajaxOptions, thrownError) {
      alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
    }
	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				if ($('select[name=\'shipping_method\']').prop('disabled') || $("input[name=same_shipping]").prop("checked")) {
...
$('select[name=\'shipping_address\']').on('change', function() {
	$.ajax({
		...
		success: function(json) {
			...
			shipping_region_id = json['region_id'];
...
var shipping_region_id = '<?php echo $shipping_region_id; ?>';
		...
		success: function(json) {
			...
			if (json['region'] && json['region'] != '') {
				...
				for (i = 0; i < json['region'].length; i++) {
					...
					if (json['region'][i]['region_id'] == shipping_region_id) {
	      				html += ' selected="selected"';
	    			}
			...
			$('#tab-shipping select[name=\'region_id\']').html(html);
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
		}
