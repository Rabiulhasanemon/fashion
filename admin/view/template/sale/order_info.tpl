    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <ul class="nav nav-tabs">
          ...
          <li><a href="#tab-assign-history" data-toggle="tab"><?php echo $tab_assign_history; ?></a></li>
          <li><a href="#tab-payment-history" data-toggle="tab"><?php echo $tab_payment_history; ?></a></li>
        ...
        <div class="tab-content">
          <div class="tab-pane active" id="tab-order">
            <table class="table table-bordered">
              ...
              <?php if ($emi) { ?>
              <tr>
                <td><?php echo $text_emi_tenure; ?></td>
                <td><?php echo $emi_tenure; ?></td>
              </tr>
              <?php } ?>
              <?php if ($ip) { ?>
              <tr>
                ...
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select name="order_status_id" id="input-order-status" class="form-control">
                      ...
                      <?php if ($order_status_it['order_status_id'] == $order_status_id) { ?>
                      <option value="<?php echo $order_status_it['order_status_id']; ?>" selected="selected"><?php echo $order_status_it['name']; ?></option>
                      <?php } else { ?>
          ...
          <div class="tab-pane" id="tab-assign-history">
            <div id="assign-history"></div>
            <br />
            <fieldset>
              <legend><?php echo $text_history; ?></legend>
              <form id="assign-history-form">
                <div class="form-group">
                  <label class="control-label" for="input-assignee"><?php echo $entry_assignee; ?></label>
                  <select name="assignee_id" id="input-assignee" class="form-control">
                    <?php foreach ($users as $user) { ?>
                    <?php if ($user['user_id'] == $assignee_id) { ?>
                ...
                <div class="row">
                  ...
                  <div class="form-group  col-md-6">
                    ...
                    <input type="checkbox" name="notify" value="1" class="form-control" />
                  </div>
                </div>
                <div class="form-group">
              ...
              <div class="text-right">
                <button id="button-change-assignee" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_change_assignee; ?></button>
              </div>
            </fieldset>
          </div>
          <div class="tab-pane" id="tab-payment-history">
            <div id="payment-history"></div>
            <fieldset>
              <legend><?php echo $text_payment_history; ?></legend>
              <form id="payment-history-form">
                <div class="row">
                  <div class="form-group col-md-6">
                  ...
                  <div class="form-group col-md-6">
                    <label for="input-payment-status"><?php echo $entry_status; ?></label>
                    <select name="payment_status" id="input-payment-status" class="form-control">
                      <option value="Pending"><?php echo $text_pending; ?></option>
                      <option value="Approved"><?php echo $text_approved; ?></option>
                      <option value="Refunded"><?php echo $text_refunded; ?></option>
                    </select>
                  </div>
                </div>
                <div class="row">
                  <div class="form-group col-md-6">
                    ...
                    <input type="text" name="amount" id="input-amount" class="form-control" placeholder="<?php echo $entry_amount; ?>">
                  </div>
                  <div class="form-group col-md-6">
                    <label for="input-transaction-id"><?php echo $entry_transaction_id; ?></label>
                    <input type="text" name="transaction_id" id="input-transaction-id" class="form-control" placeholder="<?php echo $entry_transaction_id; ?>">
                  </div>
                </div>
                <div class="row">
                  <div class="form-group  col-md-6">
              ...
              <div class="text-right">
                <button id="button-add-payment" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_add_payment; ?></button>
              </div>
            </fieldset>
          </div>
          <?php if ($payment_action) { ?>
...
$('#button-history').on('click', function() {
  if(typeof verifyStatusChange == 'function'){
	...
	$.ajax({
		...
		type: 'post',
		dataType: 'json',
		data: 'user_id=<?php echo $user_id; ?>&order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('#history-form input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('#history-form textarea[name=\'comment\']').val()),
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['success']) {
				...
				$('#history-form textarea[name=\'comment\']').val('');
...
$('#payment-history').delegate('.pagination a', 'click', function(e) {
    e.preventDefault();
    $('#payment-history').load(this.href);
});

$('#payment-history').load('index.php?route=sale/order/payment_history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');

$('#button-add-payment').on('click', function() {
    $.ajax({
        url: 'index.php?route=sale/order/addPayment&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
        type: 'post',
        dataType: 'json',
        data: new FormData($("#payment-history-form")[0]),
        cache: false,
        contentType: false,
        processData: false,
        beforeSend: function() {
        ...
        success: function(json) {
            ...
            if (json['error']) {
                $('#payment-history').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
            }

            if (json['success']) {
                $('#payment-history').load('index.php?route=sale/order/payment_history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');
                $('#history').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');
                $('#payment-history').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                $('#payment-history-form')[0].reset();
            }
        },
        error: function(xhr, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$('#assign-history').delegate('.pagination a', 'click', function(e) {
    e.preventDefault();
    $('#assign-history').load(this.href);
});

$('#assign-history').load('index.php?route=sale/assign_history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');

$('#button-change-assignee').on('click', function() {

    $.ajax({
        url: 'index.php?route=sale/assign_history/add&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>',
        type: 'post',
        dataType: 'json',
        data: new FormData($("#assign-history-form")[0]),
        cache: false,
        contentType: false,
        processData: false,
        beforeSend: function() {
            $('#button-change-assignee').button('loading');
        },
        complete: function() {
            $('#button-change-assignee').button('reset');
        },
        success: function(json) {
            $('.alert').remove();

            if (json['error']) {
                $('#assign-history').before('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
            }

            if (json['success']) {
                $('#assign-history').load('index.php?route=sale/assign_history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');
                $('#history').load('index.php?route=sale/order/history&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>');
                $('#assign-history').before('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + ' <button type="button" class="close" data-dismiss="alert">&times;</button></div>');
                $('#assign-history-form textarea[name=\'comment\']').val('');
            }
        },
        error: function(xhr, ajaxOptions, thrownError) {
            alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
