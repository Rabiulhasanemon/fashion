      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-featured" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" name="banner" value="" placeholder="<?php echo $entry_banner; ?>" id="input-banner" class="form-control" />
              <div id="featured-banner" class="well well-sm" style="height: 150px; overflow: auto;">
                <?php foreach ($banners as $banner) { ?>
                <div id="featured-banner<?php echo $banner['banner_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $banner['name']; ?>
                  <input type="hidden" name="banner[]" value="<?php echo $banner['banner_id']; ?>" />
                </div>
                <?php } ?>
              </div>
            </div>
...
$('input[name=\'banner\']').autocomplete({
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=design/banner/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
	...
	select: function(item) {
		...
		$('#featured-banner' + item['value']).remove();
		
		$('#featured-banner').append('<div id="featured-banner' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="banner[]" value="' + item['value'] + '" /></div>');
	}
});
	
$('#featured-banner').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
//--></script></div>
<?php echo $footer; ?>