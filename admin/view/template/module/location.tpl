        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-location" class="form-horizontal">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              ...
              <div id="store-location" class="well well-sm" style="height: 150px; overflow: auto;">
                <?php foreach ($locations as $location) { ?>
                <div id="store-location<?php echo $location['location_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $location['name']; ?>
...
$('input[name=\'location\']').autocomplete({
	...
	select: function(item) {
		...
		$('#store-location' + item['value']).remove();
		
		$('#store-location').append('<div id="store-location' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="location[]" value="' + item['value'] + '" /></div>');
	}
});
	
$('#store-location').delegate('.fa-minus-circle', 'click', function() {
	$(this).parent().remove();
});
//--></script></div>
<?php echo $footer; ?>