        ...
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-filter" class="form-horizontal">
          <div class="form-group required">
            ...
            <div class="col-sm-10">
              ...
              <?php if ($error_label) { ?>
              <div class="text-danger"><?php echo $error_label; ?></div>
              <?php } ?>
            </div>
          </div>
          <div class="form-group required">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" name="filter_profile" value="" placeholder="<?php echo $entry_profile; ?>" id="input-filter-profile" class="form-control" />
              <div id="group-filter-profile" class="well well-sm" style="height: 150px; overflow: auto;">
                <?php foreach ($group_filter_profiles as $group_filter_profile) { ?>
                <div id="group-filter-profile<?php echo $group_filter_profile['filter_profile_id']; ?>"><i class="fa fa-minus-circle"></i> <?php echo $group_filter_profile['name']; ?>
                  <input type="hidden" name="group_filter_profile[]" value="<?php echo $group_filter_profile['filter_profile_id']; ?>" />
                </div>
                <?php } ?>
              </div>
            </div>
...
<script type="text/javascript"><!--
    $('input[name=\'filter_profile\']').autocomplete({
        ...
        'select': function(item) {
            ...
            $('#group-filter-profile' + item['value']).remove();

            $('#group-filter-profile').append('<div id="group-filter-profile' + item['value'] + '"><i class="fa fa-minus-circle"></i> ' + item['label'] + '<input type="hidden" name="group_filter_profile[]" value="' + item['value'] + '" /></div>');
        }
    });

    $('#group-filter-profile').delegate('.fa-minus-circle', 'click', function() {
    $(this).parent().remove();
    });
  //--></script></div>
<?php echo $footer; ?> 